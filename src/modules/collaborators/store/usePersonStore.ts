...
import type { Language, Person, PersonPayload } from "../types";
...
type PersonStore = {
  ...
  createPerson: (payload: PersonPayload) => Promise<void>;
  updatePerson: (uuid: string, payload: PersonPayload) => Promise<void>;
  deletePerson: (uuid: string) => Promise<void>;
  deletePersonImage: (personUuid: string, imageUuid: string) => Promise<void>;
  togglePersonState: (uuid: string, state: boolean) => Promise<void>;
...
export const usePersonStore = create<PersonStore>((set, get) => ({
  ...
  fetchPeople: async (params = {}) => {
    try {
      ...
      const perPage = params.perPage ?? get().perPage;
      ...
      set({
        ...
        perPage: Number(paginationData?.per_page ?? perPage),
        total:
          paginationData?.total ??
          (Array.isArray(peopleData) ? peopleData.length : 0),
      });
    } catch (error) {
      console.error("Error fetchPeople:", error);
  ...
  fetchPerson: async (uuid: string) => {
    try {
      ...
      const response = await personService.getPerson(uuid);
  ...
  createPerson: async (payload: PersonPayload) => {
    try {
      ...
      showSuccess(response?.message ?? "Persona creada correctamente.");
    } catch (error) {
  ...
  updatePerson: async (uuid: string, payload: PersonPayload) => {
    try {
      ...
      const response = await personService.updatePerson(uuid, payload);

      showSuccess(response?.message ?? "Persona actualizada correctamente.");
      ...
      if (updatedPerson) {
        set((store) => ({
          selectedPerson:
            store.selectedPerson?.uuid === uuid
              ? updatedPerson
              : store.selectedPerson,
          people: store.people.map((person) =>
            person.uuid === uuid ? updatedPerson : person
          ),
        }));
      }
    } catch (error) {
  ...
  deletePerson: async (uuid: string) => {
    try {
      ...
      const response = await personService.deletePerson(uuid);

      showSuccess(response?.message ?? "Persona eliminada correctamente.");

      set((store) => ({
        people: store.people.filter((person) => person.uuid !== uuid),
        selectedPerson:
          store.selectedPerson?.uuid === uuid ? null : store.selectedPerson,
      }));
    } catch (error) {
  ...
  deletePersonImage: async (personUuid: string, imageUuid: string) => {
    try {
      set({ saving: true });

      const response = await personService.deletePersonImage(
        personUuid,
        imageUuid
      );
      ...
      set((store) => ({
        selectedPerson: store.selectedPerson
          ? {
              ...store.selectedPerson,
              images: store.selectedPerson.images?.filter((image) => {
      ...
      console.error("Error deletePersonImage:", error);
      handleApiError(error);
      throw error;
    } finally {
      set({ saving: false });
  ...
  togglePersonState: async (uuid: string, state: boolean) => {
    try {
      ...
      const response = await personService.togglePersonState(uuid, state);
      ...
      console.error("Error togglePersonState:", error);
      handleApiError(error);
      throw error;
    } finally {
      set({ loading: false });
  ...
  fetchLanguages: async (params = {}) => {
    try {
      ...
      const response = await personService.getLanguages({
        page: params.page ?? 1,
